...
struct ActiveAddFriendCardView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            invitedFriendsRef = db.collection("users").document(appStateManager.currentFilteredFroopHistory[safe: appStateManager.aFHI]?.host.froopUserID ?? ""
            ).collection("myFroops").document(appStateManager.currentFilteredFroopHistory[safe: appStateManager.aFHI]?.froop.froopId ?? "" ).collection("invitedFriends")
